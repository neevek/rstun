...
- `--hop-interval-seconds` — Optional parameter to enable connection migration by periodically changing local UDP ports at the specified interval.
...
# Start client with both TCP and UDP tunnels and connection migration every 5 minutes
rstunc \
...
Options:
  ...
  -r, --wait-before-retry-ms <MS>  Wait before retry (ms) [default: 5000]
      ...
      --hop-interval-seconds <SEC> Interval in seconds for connection migration to new random local UDP port (optional)
...
The client supports optional connection migration via the `--hop-interval-seconds` parameter. When specified, the QUIC connection will periodically migrate to a new random local UDP port at the given interval (in seconds). This feature helps avoid UDP throttling that may occur during long data transfers while maintaining the upper-layer QUIC connection without interruption.
...
**Usage:**
- If `--hop-interval-seconds` is not specified, connection migration is disabled
...
- **Connection migration**: Use `--hop-interval-seconds` to enable periodic port migration for improved performance in environments with UDP throttling.
